/* gate.js
 *
 * Client-side, code-based access gate for protected pages on david.epel.us.
 ...
 * https://david.epel.us/<name>/#code=THECODE (a bare #THECODE also works).
 ...
 *
 * On a successful unlock the code is forwarded to the decrypted page
 * (window.__GATE_CODE__ + sessionStorage) so a page that ships encrypted sidecar
 * assets under the same code can read them without a second prompt.
 *
...
(function () {
  ...
  var STORAGE_KEY = "depel-code:" + location.pathname;
  ...
  //
  // The code is handed on to the decrypted page for any encrypted sidecar assets
  // published beside it under the same code. Two channels because they fail
  // differently: document.open() keeps the same global object, so
  // window.__GATE_CODE__ survives the rewrite; sessionStorage additionally
  // survives a refresh (that's what auto-unlocks on reload).
  ...
  function buildPrompt() {
    var title = CFG.title || "david.epel.us";
    ...
    wrap.innerHTML =
      ...
      '<p class="brand">David&nbsp;<em>Epel</em></p>' +
