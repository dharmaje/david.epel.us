/* david.epel.us Admin SPA — React (UMD) + htm, no build step. */
(function () {
  "use strict";
  ...
  function getThemeOverride() { try { return sessionStorage.getItem("depel-admin-theme") || ""; } catch (e) { return ""; } }
  ...
  function ThemeToggle(props) {
    ...
    function toggle() {
      ...
      try { sessionStorage.setItem("depel-admin-theme", next); } catch (e) {}
  ...
  function claudePrompt(p) {
    return "Investigate a failed publish on david.epel.us.\n" +
      "Page slug: " + p.slug + "\nAction: " + p.kind + "\nStarted: " + new Date(p.startedAt).toISOString() +
      "\nError: " + (p.error || "publish did not complete within the polling window") +
      "\n\nContext: dharmaje/david.epel.us-internal (private) holds internal/<slug>/index.html and access-codes.xlsx; " +
      "a GitHub Action runs publish.py to encrypt the page (AES-256-GCM) and push <slug>/content.enc plus a gate shell " +
      "to dharmaje/david.epel.us (public, GitHub Pages). Check the latest workflow run logs in david.epel.us-internal, verify the " +
  ...
  function PatGate(props) {
    ...
    async function submit() {
      ...
      try {
        ...
        try { sessionStorage.setItem("depel-admin-pat", pat.trim()); } catch (e) {}
        props.onReady();
    ...
    return html`<div class="login-wrap"><div class="card login-card" style=${{ textAlign: "left" }}>
      <p class="brand" style=${{ textAlign: "center" }}>David <span>Epel</span></p>
      <p class="tag" style=${{ textAlign: "center" }}>GitHub Access</p>
      <p>Signed in as <strong>${props.user.name}</strong>. Paste a publishing token to manage pages. It is kept only for this browser session.</p>
      <p class="hint">Token scopes, for reference: Contents read/write on <code>david.epel.us-internal</code>, Contents read on <code>david.epel.us</code>.</p>
      <label for="pat">Publishing token</label>
  ...
  function Main(props) {
    ...
    return html`<div>
      <header class="hdr">
        <span class="brand">David <span>Epel</span></span>
      ...
      ${modal && modal.kind === "archive" && html`<${ConfirmModal} title="Archive page" danger=${false} confirmLabel="Archive"
        body=${"“" + modal.page.slug + "” will be unpublished from david.epel.us but its source is kept and it can be restored later. Restoring mints a new access code; notes and title are not retained (the registry row is removed)."}
      ...
      (async function () {
        if (window.AdminAuth.isConfigured) {
          ...
          if (acct) {
            ...
            var pat = null;
            try { pat = sessionStorage.getItem("depel-admin-pat"); } catch (e) {}
            if (pat) {
